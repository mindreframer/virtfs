defmodule Virtfs.TypedStruct do
  @accumulating_attrs [
    :ts_plugins,
    :ts_plugin_fields,
    :ts_fields,
  ...
  defmacro __using__(_) do
    quote do
      import Virtfs.TypedStruct, only: [typedstruct: 1, typedstruct: 2]
    end
  end
  ...
  defmacro typedstruct(opts \\ [], do: block) do
    ast = Virtfs.TypedStruct.__typedstruct__(block, opts)
  ...
  def __typedstruct__(block, opts) do
    quote do
      ...
      import Virtfs.TypedStruct
      unquote(block)

      @enforce_keys @ts_enforce_keys
      defstruct @ts_fields

      Virtfs.TypedStruct.__type__(@ts_types, unquote(opts))
    end
  end
  ...
  defmacro field(name, type, opts \\ []) do
    quote bind_quoted: [name: name, type: Macro.escape(type), opts: opts] do
      Virtfs.TypedStruct.__field__(name, type, opts, __ENV__)
    end
  end
